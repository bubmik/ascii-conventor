...
public:
    ASCIIVideoPlayer(const std::string& path)
        : videoPath(path), frameRate(30), consoleWidth(80), consoleHeight(25),
    ...
    void playStreaming() {
        ...
        while (true) {
            ...
            DWORD written;
            WriteConsoleA(hConsole, asciiBuffer, strlen(asciiBuffer), &written, NULL);
...
int main() {
    ...
    std::cout << "=== ASCII Video Player - TRUE FULLSCREEN ===\n\n";

    const std::string videoFile = "ascii-convert.mp4";

    std::ifstream file(videoFile);
    if (!file.good()) {
        std::cerr << "Error: 'ascii-convert.mp4' not found!\n";
        system("pause");
        return 1;
    }
    file.close();

    ASCIIVideoPlayer player(videoFile);

    std::cout << "Ready! Press Enter to start fullscreen playback...\n";
    std::cin.get();
